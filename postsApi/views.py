...
class PostCreateView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            for i in image_keys:
                imageModel = Image()
                image_data = request.data[i]
                format, imgstr = image_data.split(';base64,')
                ext = format.split('/')[-1]
                data = ContentFile(base64.b64decode(imgstr))
                file_name = "postImage." + ext

                imageModel.name=f'{title} {radon_title} {auhtor} {date_time_key}'
        ...
        try:
            image_data = request.data['coverImg']
            format, imgstr = image_data.split(';base64,')
            ext = format.split('/')[-1]
            data = ContentFile(base64.b64decode(imgstr))
            file_name = "coverimage." + ext

            post.cover_image.save(file_name, data, save=True)
        except:
            pass
        post.save()
...
class PostUpdateView(APIView):
    ...
    def post(self, request, pk, format=None):
        ...
        # Start saving the images in models
        try:
            # get images and save as objects
            image_keys = list(request.data)[-int(request.data["imglength"]):]
...
class CommentsView(APIView):
    ...
    def post(self, request, pk, format=None):
        ...
        comment_id = Comments.objects.filter(
            name=f'{comment_description} {profile} {date_time_key} {radon_title}')
...
class ReplesView(APIView):
    ...
    def post(self, request, pk, format=None):
        ...
        reply_id = Replyes.objects.filter(
            name=f'{comment_description} {profile} {date_time_key} {radon_title}')
